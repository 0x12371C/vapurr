...
/// Upgradeable House swapper — UUPS behind ERC1967Proxy.
/// Exact-in swap on the house Uniswap v4 $VAPURR / $PUSD pool.
/// Matches the shape actually live on 46630 (pre-wgV/HousePairConfig refactor,
/// commit e3b3565) — the live TESTNET_SWAP predates `ef87976`'s wgV switch, so
/// this carries that same live behavior forward under a proxy rather than
/// silently adopting the newer wgV/HousePairConfig design, which the team's
/// own docs/econ/TESTNET_ROLLOUT.md §9 still marks "still open" / not deployed.
/// Stateless besides transient in-flight balances — nothing to migrate.
...
contract HouseSwapUpgradeable is Initializable, UUPSUpgradeable {
    ...
    function initialize(address market_, address pm_, uint24 fee_, int24 tickSpacing_, address owner_) external initializer {
        require(market_ != address(0) && pm_ != address(0) && owner_ != address(0), "MKT");
    ...
    function houseSwapVersion() external pure returns (uint256) {
        return 1;
    }
