...
/// Upgradeable PNS — UUPS behind ERC1967Proxy. Purr Name Service, TLD .hood,
/// same way ENS holds .eth. Same logic as the live bare-impl `PnsRegistry.sol`
/// (`0x13C9fCaB…0fC4`) — that instance predates this proxy and its own
/// registrations do not carry over (a first-time proxy-ification always
/// starts empty; there is no upgrade path from a contract that was never a
/// proxy). Root node (0x00) owner is the upgrade authority (`_authorizeUpgrade`),
/// same as it always was the registry's admin — no separate owner var added.
contract PnsRegistryUpgradeable is Initializable, UUPSUpgradeable {
    ...
    function initialize(address rootOwner) external initializer {
        require(rootOwner != address(0), "OWNER");
        recs[ROOT_NODE].owner = rootOwner;
        recs[HOOD_NODE].owner = address(this);
        names[HOOD_NODE] = "hood";
        emit Transfer(ROOT_NODE, rootOwner);
        emit NewOwner(ROOT_NODE, keccak256("hood"), address(this));
        emit Transfer(HOOD_NODE, address(this));
    }

    function _authorizeUpgrade(address) internal view override {
        require(recs[ROOT_NODE].owner == msg.sender, "OWNER");
    }

    /// Surface marker for upgrade proofs (v1).
    function pnsVersion() external pure returns (uint256) {
        return 1;
    }
