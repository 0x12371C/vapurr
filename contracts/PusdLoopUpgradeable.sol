...
/// Upgradeable Oliver (PusdLoop) — UUPS behind ERC1967Proxy.
...
/// Storage-compatible economic surface with the non-proxy PusdLoop;
/// owner/market/vapurr/pusd are storage (not immutable) so they survive upgrades.
/// Deploy shape required by docs/econ/PROXY_DEPLOY_GATE.md: impl first, then
/// ERC1967Proxy(impl, initialize(...)) — never wire the bare impl as live.
...
contract PusdLoopUpgradeable is Initializable, UUPSUpgradeable {
    ...
    /// 2026-09-06: lowered 150%->30% (was 15e17). At 150% no rational borrower
    /// ever showed up, so real cash never crossed BOOT_CASH, so the rate never
    /// faded — a permanent deadlock (live gen-4 vault: ~0.0007 PUSD total cash
    /// after two days). 30% still taxes boot-phase borrowing well above the 6%
    /// steady rate without being prohibitive. loop() already cannot invent
    /// depth past real cash (min(room, cash)), so this does not reopen that hole.
    uint256 public constant BOOT_SLOPE1 = 3e17;
    uint256 public constant SLOPE2 = 1e18;
    /// $PUSD cash at which boot slope has fully faded to BASE_SLOPE1.
    /// 2026-09-06: lowered 100,000->5,000 PUSD for the same reason — 100k was
    /// never going to arrive while the vault was inert. 5k is a target real
    /// supply can plausibly clear, so the boot actually fades instead of
    /// sitting at the ceiling indefinitely.
    /// Looping does not raise cash — only real supply / Lithe drip does.
    uint256 public constant BOOT_CASH = 5_000 * DEC;
    ...
    /// Reserved for future Oliver storage (upgrade-safe).
    ...
    function initialize(address market_, address owner_) external initializer {
        require(market_ != address(0) && owner_ != address(0), "MKT");
        owner = owner_;
