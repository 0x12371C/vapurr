...
const src = fs.readFileSync(path.join(dir, "HouseLp.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "HouseLp.sol": { content: src } },
  settings: {
...
const c = out.contracts["HouseLp.sol"].HouseLp;
const hex = c.evm.bytecode.object;
if (!hex || hex.length < 200) {
  console.error("empty bytecode");
  process.exit(1);
}
const dest = path.join(dir, "..", "crates", "vapurr-econ", "src", "house.hex");
