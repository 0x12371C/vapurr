...
const src = fs.readFileSync(path.join(dir, "KetList.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "KetList.sol": { content: src } },
  settings: {
...
const c = out.contracts["KetList.sol"].KetList;
const hex = c.evm.bytecode.object;
if (!hex || hex.length < 200) {
  console.error("empty bytecode");
  process.exit(1);
}
const dest = path.join(dir, "..", "crates", "vapurr-econ", "src", "ketlist.hex");
