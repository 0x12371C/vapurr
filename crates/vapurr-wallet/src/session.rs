...
pub fn status() -> Value {
    json!({"ok":true,"logged_in":is_logged_in(),"has_key":has_key(),"address":peek_address().unwrap_or_default()})
}
...
pub fn reveal_seed() -> Result<Value, WalletError> {
    ...
    Ok(json!({"ok":true,"seed":seed,"logged_in":true,"has_key":true,"address":peek_address().unwrap_or_default()}))
}
pub fn export_key() -> Result<Value, WalletError> {
    ...
    Ok(json!({"ok":true,"hex_key":format!("0x{}", hex::encode(raw.as_slice())),"logged_in":true,"has_key":true,"address":key.address.to_checksum()}))
}
pub fn logout() -> Value {
    UNLOCKED.store(false, Ordering::Release);
