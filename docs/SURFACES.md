...
| `id` | | `id.html` | zer0ID. Live state from `/id/api/status` — verified level, handle, expiry, and the 0–4 ladder. Staged ocular flow: consent → phone (L3 prerequisite) → QR hand-off to an iPhone → poll until the attestation verifies. Reports verified level only; a file claiming more is flagged, never trusted. Does not fake Proven. Not Shield |
