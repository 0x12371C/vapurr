...
| Build / pack | House | **Programs 1.1.15** (VERSION.txt; sha `F7436BF66BCC`, rev `e590ef3`, **NotSigned**); AppData channel **1.1.15** (same rev/sha as VERSION); **TSL channel live 1.1.13** (rev `e590ef3`, sha `324286bf6f68...`); dist pack artifacts still **1.1.10**; local repo `channel/manifest.json` missing. Gasless relay + route catalog seed on `pack/d59912d` (overnight 15:06); SNAPSHOT honesty 16:09 (Programs/AppData 1.1.15 vs TSL 1.1.13; rebate lore dropped). Hold stranger-ship until signed + Relic security bar. | Signed pack to TSL after Relic retest; promote TSL channel to 1.1.15 to match Programs/AppData; SignPath remains ship P0 |
