...
| Build / pack | House | **Programs 1.1.17** (VERSION.txt; sha `93A4AAE04C7C`, rev `e590ef3`, **NotSigned**); AppData channel **1.1.17** (same rev/sha); **TSL channel live 1.1.13** (TLS restored; rev `e590ef3`, sha `324286bf6f68...`); dist pack artifacts still **1.1.10**; local repo `channel/manifest.json` missing. Gasless preview on `pack/d59912d`; overnight 18:14 fixed `/relay/quote` solo base 21k→`EVM_BASE_TX_GAS` 25732 + `verify-gasless.py`. Hold stranger-ship until signed + Relic security bar. | Signed pack to TSL after Relic retest; promote TSL channel to 1.1.17 to match Programs/AppData; SignPath remains ship P0; Uninstall DisplayVersion now follows VERSION.txt on patch |
