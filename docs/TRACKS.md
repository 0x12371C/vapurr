...
| Build / pack | House | **Programs 1.1.14** (VERSION.txt; sha `F5897A8BEC53`, rev `e590ef3`, **NotSigned**); Programs `manifest.json` still **1.1.13** (same rev; size/sha differ); dist pack artifacts **1.1.10**; TSL channel manifest **404**; local repo `channel/manifest.json` missing. Gasless relay source `6385b67` on `pack/d59912d`. Hold public until signed + Relic security bar. | Signed pack to TSL after Relic retest; restore public channel manifest; reconcile Programs manifest vs VERSION |
