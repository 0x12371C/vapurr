...
- BondMarket gated skeleton + HouseFeeRemit sketch + FeeAttribution ledger landed (P1 live enable still open; Uni skim adapter HouseUniSkim landed) - sPUSD CD sketch + Bonds `#spusd-cd` UI stub landed; Bonds live-by-default CTAs + params banner landed; stock session/corp-action Fed ops + UI session stub landed (live wire/address book + auto calendar still open); House tab **wgV / $PUSD** visual stub on `pusd.html` (wrap-first gate; live pairConfig deploy still open); **shared `#routing-visual` product map** on Cash (`pusd.html`) + Bonds (`bonds.html`) via `defi-flow.js` / `defi-flow.css` (lane clicks: Cash->Lithe, Equity/Bonds->bonds, House->house; highlight follows desk)
