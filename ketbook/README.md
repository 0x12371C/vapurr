...
You browse on this PC. Robinhood Chain is home. **$PUSD** is what the window spends (KetPay, postage, vapurrbid). **$VAPURR** is burned to mint that dollar. **Lithe** is 9% on $PUSD.
...
The loop: [The loop](loop.md). Supply and borrow (Euler-shaped `$PUSD` vault) is [here](euler.md) — empty CA until deploy.
