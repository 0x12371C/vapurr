...
Live on testnet 46630 at `0x07d1085b545d5e1f55668a6a2EA9332233AaeC69` (EIP-1967 proxy — `PusdLoopUpgradeable` behind `ERC1967Proxy`, impl `0x982f20837FBC9112225504580e0d9dc23b3eAAa5`; see `docs/econ/PROXY_DEPLOY_GATE.md`). Mainnet is empty. The desk shows the vault. Old non-upgradeable `0x89E17eef…4521` is retired.
...
| `BOOT_SLOPE1` | 30% | borrow APY at kink while cash is 0 (was 150% — that never fully faded, see below) |
| `BASE_SLOPE1` | 6% | borrow APY at kink once 5,000 $PUSD cash is in |
| `BOOT_CASH` | 5,000 P | exogenous cash that fades the boot. Looping does not count. (was 100,000 — the old target was never reached) |
...
Below the kink, borrow APY rises linearly to the live slope. That slope starts at **30%** when vault cash is 0 and fades to **6%** as real `$PUSD` cash hits 5,000. Recursive loop does not raise cash, so it cannot farm the boot forever. Real suppliers and Lithe drip can. Above the kink, SLOPE2 still jumps. Supply APY is borrow APY × util × (1 − reserve). Lithe is separate: 9% on vault-held `$PUSD` cash.
