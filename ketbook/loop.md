...
**Supply and borrow** (Euler-shaped, not a Euler fork) is how that dollar gets its own depth. Isolated `$PUSD` credit. `$VAPURR` is collateral. Supply P, borrow P, loop under 85% LTV. Utilization sets the rate. Liquidations at 90%. Lithe still drips on vault-held `$PUSD`. Vault `0x07d1085b…eC69` (EIP-1967 proxy, `PusdLoopUpgradeable`) is live on 46630. Boot kink 30% → 6% as 5,000 cash lands. See [Supply and borrow](euler.md).
